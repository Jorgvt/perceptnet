...
NOISE_VAR = 0.25
try:
    filt = state.state["precalc_filter"]["GaborLayerGammaRepeat_0"]["kernel"][...,0,FILTER_IDX]
except:
    filt = state.state["precalc_filter"]["CenterSurroundLogSigmaK_0"]["kernel"][...,0,FILTER_IDX]
filt.shape
...
# %%
final_imgs = []
for FILTER_IDX in tqdm(range(state.state["precalc_filter"]["GaborLayerGammaRepeat_0"]["kernel"].shape[-1])):
    ...
    axes[1].imshow(img[0])
    axes[2].imshow(state.state["precalc_filter"]["GaborLayerGammaRepeat_0"]["kernel"][...,0,FILTER_IDX])
    axes[3].plot(losses)
