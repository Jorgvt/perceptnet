...
# %%
K_as = jnp.linspace(1.1, 5, num=10)
K_ts = jnp.linspace(1.1, 5, num=10)
K_ds = jnp.linspace(1.1, 5, num=10)
...
for K_a, K_t, K_d in tqdm(product(K_as, K_ts, K_ds), total=len(K_as)*len(K_ts)*len(K_ds)):
    ## Insert the parameter values into the model
    params = unfreeze(state.params)
    params["CenterSurroundLogSigmaK_0"]["K"] = jnp.array([K_a, K_a, K_a,
                                                          K_t,K_t,K_t,
                                                          K_d, K_d, K_d])
    ...
    results["K_t"].append(K_t)
    results["K_d"].append(K_d)
    # break
results
...
# %%
pd.DataFrame(results).to_csv("results_CenterSurround.csv", index=False)
