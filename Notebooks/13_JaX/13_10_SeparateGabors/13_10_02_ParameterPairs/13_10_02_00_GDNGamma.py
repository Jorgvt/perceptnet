...
# %%
biases = jnp.linspace(0., 1., num=10)
kernels = jnp.linspace(0., 1., num=10)
...
for bias, kernel in tqdm(product(biases, kernels), total=len(biases)*len(kernels)):
    ## Insert the parameter values into the model
    params = unfreeze(state.params)
    params["GDNGamma_0"]["bias"] = params["GDNGamma_0"]["bias"].at[0].set(bias)
    params["GDNGamma_0"]["kernel"] = params["GDNGamma_0"]["kernel"].at[0].set(kernel)
...
# %%
pd.DataFrame(results).to_csv("results_GDNGamma.csv", index=False)
