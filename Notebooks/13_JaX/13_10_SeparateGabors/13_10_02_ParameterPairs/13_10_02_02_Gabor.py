...
for f_min, spread in tqdm(product(f_mins, spreads), total=len(f_mins)*len(spreads)):
    ...
    params["GaborLayerGammaHumanLike__0"]["freq_a"] = jnp.linspace(f_min, f_min+spread, num=4) # jnp.array([2.,4.,8.,16.]) 
...
# %%
pd.DataFrame(results).to_csv("results_GaborA.csv", index=False)