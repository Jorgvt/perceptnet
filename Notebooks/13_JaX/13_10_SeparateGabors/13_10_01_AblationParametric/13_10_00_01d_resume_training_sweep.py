...
parser = argparse.ArgumentParser(description="Recover a given run and keep training.",
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument("--id", help="id of run to be resumed.")
...
## Download files from previous run
api = wandb.Api()
prev_run = api.run(f"jorgvt/PerceptNet_v15/{wandb.run.id}")
...
class PerceptNet(nn.Module):
    ...
    def __call__(self,
        ...
        if config.FINAL_B: B = self.param("B", nn.initializers.ones_init(), (outputs.shape[-1],))
        else: B = 1.
        return B*outputs
